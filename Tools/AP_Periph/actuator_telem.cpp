...
void ActuatorTelem::send_telemetry(uint8_t channel_index, uint8_t actuator_id)
{
    ...
    const bool have_duty_cycle = (max_current > 0) && (current_amp >= 0);
    if (have_duty_cycle) {
        duty_cycle = constrain_float(current_amp / max_current * 100.0f, 0, 100);
    }
    ...
    const AP_Servo_Telem::TelemetryData telem_data {
        ...
        .present_types = AP_Servo_Telem::TelemetryData::Types::CURRENT |
                         have_duty_cycle ? AP_Servo_Telem::TelemetryData::Types::DUTY_CYCLE : 0
    };

    periph.servo_telem.lib.update_telem_data(channel_index, telem_data);
