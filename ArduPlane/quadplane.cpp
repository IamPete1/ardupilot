...
#define FRAME_CLASS_OLD_IDX 30
    ...
    // @Values: 0:Plus, 1:X, 2:V, 3:H, 4:V-Tail, 5:A-Tail, 10:Y6B, 11:Y6F, 12:BetaFlightX, 13:DJIX, 14:ClockwiseX, 15:I (Octa only), 16:MOTOR_FRAME_TYPE_NYT_PLUS, 17:MOTOR_FRAME_TYPE_NYT_X, 18:X_X (Octa only)
    // @User: Standard
    ...
    // @Bitmask: 0:LevelTransition,1:AllowFWTakeoff,2:AllowFWLand,3:Respect takeoff frame types,4:Use a fixed wing approach for VTOL landings,5:Use QRTL instead of QLAND for failsafe when in VTOL modes,6:Use idle governor in MANUAL,7:QAssist force enabled
...
const AP_Param::GroupInfo QuadPlane::var_info2[] = {
    ...
    // @Param: TAILSIT_OPTION
    // @DisplayName: Tailsitter options bitmask
    // @Description: Tailsitter options bitmask
    // @Bitmask: 0:QAssist motors only
    // @User: Standard
    AP_GROUPINFO("TAILSIT_OPTION", 19, QuadPlane, tailsitter.options, 0),
...
bool QuadPlane::setup(void)
{
    ...
    if (options & OPTION_Q_ASSIST_FORCE_ENABLE) {
