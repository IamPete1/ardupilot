...
void ModeQAcro::run()
{
    if (plane.quadplane.throttle_wait) {
        ...
        float target_roll = 0;
        float target_pitch = plane.channel_pitch->norm_input() * plane.quadplane.acro_pitch_rate * 100.0f;
        float target_yaw = 0;
        if (plane.quadplane.is_tailsitter()) {
            // Note that the 90 degree Y rotation for copter mode swaps body-frame roll and yaw
            target_roll =  plane.channel_rudder->norm_input() * plane.quadplane.acro_yaw_rate * 100.0f;
            target_yaw  = -plane.channel_roll->norm_input() * plane.quadplane.acro_roll_rate * 100.0f;
        } else {
            target_roll = plane.channel_roll->norm_input() * plane.quadplane.acro_roll_rate * 100.0f;
        ...
        plane.quadplane.attitude_control->set_throttle_out(throttle_out, false, 10.0f);
    }
}
...
void ModeQLand::init()
{
    ...
    plane.quadplane.poscontrol.set_state(QuadPlane::QPOS_LAND_DESCEND);
...
void ModeQLoiter::run()
{
    ...
    if (plane.control_mode == &plane.mode_qland) {
        if (plane.quadplane.poscontrol.get_state() < QuadPlane::QPOS_LAND_FINAL && plane.quadplane.check_land_final()) {
            plane.quadplane.poscontrol.set_state(QuadPlane::QPOS_LAND_FINAL);
            plane.quadplane.setup_target_position();
        ...
        if (plane.quadplane.poscontrol.get_state() == QuadPlane::QPOS_LAND_FINAL && (plane.quadplane.options & QuadPlane::OPTION_DISABLE_GROUND_EFFECT_COMP) == 0) {
            plane.quadplane.ahrs.set_touchdown_expected(true);
        }
...
void ModeQRTL::run()
{
    plane.quadplane.vtol_position_controller();
    if (plane.quadplane.poscontrol.get_state() >= QuadPlane::QPOS_POSITION2) {
        // change target altitude to home alt
        plane.next_WP_loc.alt = plane.home.alt;
        plane.quadplane.verify_vtol_land();
    }
}
...
void ModeQRTL::init()
{
    ...
    plane.quadplane.pos_control->set_accel_desired_xy_cmss(Vector2f());
    plane.quadplane.pos_control->init_xy_controller();
    plane.quadplane.poscontrol_init_approach();
    ...
    if (dist < 1.5*radius &&
        plane.quadplane.motors->get_desired_spool_state() == AP_Motors::DesiredSpoolState::THROTTLE_UNLIMITED) {
        ...
        plane.quadplane.poscontrol.set_state(QuadPlane::QPOS_POSITION1);
    }
    int32_t from_alt;
    int32_t to_alt;
    if (plane.current_loc.get_alt_cm(Location::AltFrame::ABSOLUTE,from_alt) && plane.next_WP_loc.get_alt_cm(Location::AltFrame::ABSOLUTE,to_alt)) {
        plane.quadplane.poscontrol.slow_descent = from_alt > to_alt;
        return;
    }
    // defualt back to old method
    plane.quadplane.poscontrol.slow_descent = (plane.current_loc.alt > plane.next_WP_loc.alt);
}
