...
void AP_WindVane::update()
{
    ...
    */
    ...
    if (is_positive(_speed_filt_hz)) {
        ...
        _speed_apparent = _speed_apparent_filt.apply(_speed_apparent_raw);
    } else {
    ...
    if (is_positive(_dir_filt_hz)) {
        ...
        filtered_sin = _direction_apparent_sin_filt.apply(sinf(_direction_apparent_raw));
        filtered_cos = _direction_apparent_cos_filt.apply(cosf(_direction_apparent_raw));
    ...
    filtered_sin = _tack_sin_filt.apply(sinf(_direction_apparent_raw));
    filtered_cos = _tack_cos_filt.apply(cosf(_direction_apparent_raw));
    ...
    if (is_positive(_true_filt_hz)) {
        ...
        _speed_true = _speed_true_filt.apply(_speed_true_raw);
        filtered_sin = _direction_true_sin_filt.apply(sinf(_direction_true_raw));
        filtered_cos = _direction_true_cos_filt.apply(cosf(_direction_true_raw));
