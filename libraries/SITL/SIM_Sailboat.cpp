...
{
    if (sitl->wave.enable == 0) {
        wave_phase = 0.0f;
        wave_gyro = Vector3f(0.0f , 0.0f, 0.0f);
        wave_heave = 0.0f;
        return;
    }
    ...
    if (!hal.util->get_soft_armed() || is_zero(wave_amp) ) { 
...
void Sailboat::update(const struct sitl_input &input)
{
    ...
    if ((sitl->sailboat_options & (1 << 0)) != 0) {
        throttle_force = (input.servos[THROTTLE_SERVO_CH]-1500) * 0.1f;
    }
