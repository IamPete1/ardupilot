...
*/
void JSON_generic::set_interface_ports(const char* address, const int port_in, const int port_out)
{
...
*/
void JSON_generic::output_servos(const struct sitl_input &input)
{
    servo_packet pkt;
    pkt.frame_time_us = frame_time_us;

    for (uint8_t i=0; i<16; i++) {
        pkt.pwm[i] = input.servos[i];
    }

    size_t send_ret = sock.sendto(&pkt, sizeof(pkt), airsim_ip, airsim_control_port);
...
/*
  ...
  general purpose
*/
bool JSON_generic::parse_sensors(const char *json)
{
    //printf("%s\n", json);
...
#if 0
// @LoggerMessage: JSN1
// @Description: Log data received from simulator over generic JSON
...
*/
void JSON_generic::update(const struct sitl_input &input)
{
    output_servos(input);
...
/*
  report frame rates
 */
void JSON_generic::report_FPS(void)
{
