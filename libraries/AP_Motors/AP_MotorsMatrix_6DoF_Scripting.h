...
public:
    ...
    void add_motor(int8_t motor_num, float roll_factor, float pitch_factor, float yaw_factor, float throttle_factor, float forward_factor, float right_factor, uint8_t testing_order);

    // if the expected number of motors have been setup then set as initalized
    bool initialize(uint8_t expected_num_motors);
