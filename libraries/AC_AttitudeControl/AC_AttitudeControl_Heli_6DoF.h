...
class AC_AttitudeControl_Heli_6DoF : public AC_AttitudeControl_Heli {
public:
    AC_AttitudeControl_Heli_6DoF(AP_AHRS_View &ahrs, const AP_MultiCopter &aparm, AP_MotorsHeli& motors):
        AC_AttitudeControl_Heli(ahrs,aparm,motors) {

        if (_singleton != nullptr) {
            AP_HAL::panic("Can only be one AC_AttitudeControl_Heli_6DoF");
        }
        _singleton = this;
    }

    static AC_AttitudeControl_Heli_6DoF *get_singleton() {
        return _singleton;
    }
...
private:
    ...
    static AC_AttitudeControl_Heli_6DoF *_singleton;
