...
local function cam_update()
   ...
   if cam_state=='DISCONNECTED' or cam_rtsp == nil then
      ...
      if now - last_retry_ms > VID_RETRY_MS:get() then
         last_retry_ms=now
         ...
         if connected then
            ...
            rtsp_send(cam_rtsp,'OPTIONS',cam_uri(),cseq_cam,{cam_auth(),'User-Agent: ArduPilot-Lua'}); cseq_cam=cseq_cam+1
         else
   ...
   if cam_state=='STREAMING' and (now - last_ka_cam > 25000) then
      rtsp_send(cam_rtsp,'GET_PARAMETER',cam_uri(),cseq_cam,{cam_auth(),'User-Agent: ArduPilot-Lua', cam_session and ('Session: '..cam_session) or ''}); cseq_cam=cseq_cam+1; last_ka_cam=now
   end
end
   ...
   if mtx_state=='DISCONNECTED' or mtx_rtsp == nil then
      if cam_state=='STREAMING' then
         ...
         if connected then
            ...
            rtsp_send(mtx_rtsp,'ANNOUNCE',mtx_uri_base(),cseq_mtx,{mtx_auth(),'User-Agent: ArduPilot-Lua'}, sdp); cseq_mtx=cseq_mtx+1
         else
      ...
      if m.kind=='rtsp' then
         if m.code == 200 then
            if mtx_state=='ANNOUNCE' then
               ...
               rtsp_send(mtx_rtsp,'SETUP',track,cseq_mtx,{mtx_auth(),'User-Agent: ArduPilot-Lua','Transport: RTP/AVP/TCP;unicast;interleaved=0-1;mode=record'}); cseq_mtx=cseq_mtx+1; mtx_state='SETUP'
