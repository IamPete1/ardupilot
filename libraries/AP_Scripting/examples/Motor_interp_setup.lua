...
for i = 1, #interpolation_points do
    ...
    -- load the table
    add_motors_interp_table(interpolation_points[i],mechanical_gain_scaling[i])
end
...
function interp_table(tilt, lookup_table)
...
function update_interpolation()

    -- update motors interpolation point from the script in real-time
    Motors_interp:set_interpolation_point(current_tilt)
