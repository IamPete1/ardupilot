...
bool command_long_buffer::write(const mavlink_command_long_t &cmd, const uint32_t time_ms, const mavlink_channel_t chan) {
    WITH_SEMAPHORE(sem);
    bool ret = false;
    if (cmd.command == watch) {
        struct scripting_cmd_long data {cmd, time_ms, chan};
        buffer.push(data);
        ret = true;
    }
    if (next != nullptr) {
...
bool command_long_buffer::receive(mavlink_command_long_t &cmd, uint32_t &time_ms, uint8_t &chan) {
    WITH_SEMAPHORE(sem);
...
// write item to buffer if it is the watched command
bool command_int_buffer::write(const mavlink_command_int_t& cmd, const uint32_t time_ms, const mavlink_channel_t chan) {
    WITH_SEMAPHORE(sem);
    bool ret = false;
    if (cmd.command == watch) {
        struct scripting_cmd_int data {cmd, time_ms, chan};
        buffer.push(data);
        ret = true;
    }
    if (next != nullptr) {
...
// pop command from buffer, save last channel for ack
bool command_int_buffer::receive(mavlink_command_int_t & cmd, uint32_t &time_ms, uint8_t &chan) {
    WITH_SEMAPHORE(sem);
