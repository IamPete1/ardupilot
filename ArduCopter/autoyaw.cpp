...
void Mode::AutoYaw::update_weathervane(const int16_t roll_cdeg, const int16_t pitch_cdeg, const int16_t pilot_yaw, const int32_t hgt_cm)
{
    ...
    if (copter.g2.weathervane.should_weathervane(pilot_yaw, (float)hgt_cm*0.01)) {
        ...
        if (copter.ap.land_complete || copter.ap.land_complete_maybe) {
            ...
            const float limit = MIN(copter.g2.pilot_y_rate.get() * 100.0, copter.attitude_control->get_slew_yaw_cds()) * 0.5;
            _yaw_rate_cds = constrain_float(copter.g2.weathervane.get_yaw_rate_cds(roll_cdeg, pitch_cdeg), -limit, limit);
        }
